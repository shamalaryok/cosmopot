...
class ReferralService:
    ...
    async def get_referral_code(self, session: AsyncSession, user: UserProtocol) -> str:
        ...
        stmt = (
            ...
            .where(Referral.referrer_id == cast(uuid.UUID, user.id))
            .limit(1)
        )
    ...
    async def get_referral_stats(
        ...
        earnings_stmt = select(
            func.coalesce(func.sum(ReferralEarning.amount), Decimal("0"))
        ...
        withdrawals_stmt = select(
            func.coalesce(func.sum(ReferralWithdrawal.amount), Decimal("0"))
        ).where(
            ReferralWithdrawal.user_id == cast(uuid.UUID, user.id),
            ReferralWithdrawal.status == WithdrawalStatus.PROCESSED,
        )
        total_withdrawn = (await session.execute(withdrawals_stmt)).scalar() or Decimal(
            "0"
        )
    ...
    async def process_referral_code(
        ...
        stmt = select(Referral).where(
            Referral.referral_code == referral_code, Referral.is_active
        )
    ...
    async def get_user_withdrawals(
        ...
        stmt = (
            ...
            .where(ReferralWithdrawal.user_id == cast(uuid.UUID, user.id))
